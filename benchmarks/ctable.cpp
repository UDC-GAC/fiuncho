 ...
 * @brief Benchmarking program for the contigency table computation function
 * @date 20/09/2021
 */
...
/**
 * @brief Contingency table calculation loop. The function repeats, for a given
 * number of iterations, the computation of the contingency tables for {SNP_0,
 * ..., SNP_o-2, SNP_i}, with o being the order of the contingency tables and i
 * in [o-1, dataset.snps].
 *
 * @param repetitions Number of iterations to loop
 * @param dataset Input data
 * @param order Order of the contingency tables computed
 * @param prev Genotype table of combination {SNP_0, ..., SNP_o-2}
 */

void loop_ctable(const int repetitions, const Dataset<uint64_t> &dataset,
 ...
 * elapsed time during the calculation of the contingency table of the
 * combination {SNP_0, ..., SNP_o-2, SNP_i}, with o being the order of the
 * contingency tables and i in [o-1, dataset.snps]. The function creates as many
 * threads as CPU cores indicated in the affinity, and pins them to the
 * specified allocation.
 *
 * @param tped TPED file path
 * @param tfam TFAM file path
 * @param order Order of the contingency tables computed by the benchmark
...
std::vector<double> bench_ctable(const std::string tped, const std::string tfam,
                                 const unsigned short order,
                                 ...
                                 const int repetitions)
{
    ...
    if (order == 2) {
        return multithread_pinned_time(affinity, loop_ctable, repetitions,
...
int main(int argc, char *argv[])
{
    ...
    // Run benchmark
    auto times = bench_ctable(tped, tfam, order, affinity, repetitions);
    // Print times
