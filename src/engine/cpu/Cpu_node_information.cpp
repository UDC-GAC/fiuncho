...
Cpu_node_information::Cpu_node_information(const void *pointer)
    : Node_information() {
...
std::string Cpu_node_information::hostname() const { return hostname_str; }

std::string Cpu_node_information::cpu() const { return cpu_str; }

long Cpu_node_information::memory() const { return memory_size; }
...
std::string Cpu_node_information::to_string() const {
    ...
    output += "CPU: " + cpu_str + "\n";
    output +=
        "Memory: " + std::to_string(memory_size / (1024 * 1024)) + " MB\n";
...
size_t Cpu_node_information::to_byteblock(void **pointer) const {
    const size_t buffer_size = 5 * sizeof(size_t) + hardware_id.length() +
                               hostname_str.length() + cpu_str.length() +
                               sizeof(unsigned int) + mpi_library.length() +
                               process_list.size() * sizeof(int);
