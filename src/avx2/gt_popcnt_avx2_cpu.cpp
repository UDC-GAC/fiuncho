...
inline void iter(const uint64_t *ptr1, const uint64_t *ptr2, uint32_t &result)
{
    ...
    __m256i inter = _mm256_and_si256(o1, o2);
    ...
    for (i = 0; i < gt_size; ++i) {
        for (j = 0; j < 3; ++j) {
            ...
            for (k = 0; k + 16 <= words; k += 16) {
                iter(gt_tbl1 + k + 0, gt_tbl2 + k + 0, result);
                iter(gt_tbl1 + k + 4, gt_tbl2 + k + 4, result);
                iter(gt_tbl1 + k + 8, gt_tbl2 + k + 8, result);
                iter(gt_tbl1 + k + 12, gt_tbl2 + k + 12, result);
            }
            for (; k < words; k += 4) {
                iter(gt_tbl1 + k, gt_tbl2 + k, result);
            }
            ct_tbl[i * 3 + j] = result;
        }
    }
